...
import { Mail, ArrowRight, X, AlertCircle, CheckCircle2, Lock, Eye, EyeOff } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
    ...
    subtitle = "Inserisci la tua email e password per sincronizzare i tuoi obiettivi nel cloud."
}) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isResetPassword) {
            try {
                ...
                setStatus('error');
                setErrorMessage(err.message || 'Errore durante la richiesta di reset della password.');
            }
            return;
        }
        ...
        try {
            if (isSignUp) {
                ...
                if (error) throw error;
                // If "Confirm Email" is disabled, they are logged in automatically.
                onSuccess?.();
                onClose();
            } else {
                    ...
                    >
                        ...
                        <div className="text-center mb-8">
                            ...
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
                                {isResetPassword ? 'Recupera Password' : title}
                            </h2>
                            <p className="text-slate-600 dark:text-slate-400 text-sm">
                                {isResetPassword 
                                    ? "Riceverai una mail per impostare una nuova password." 
                                    : subtitle}
                            </p>
                        </div>

                        {status === 'success' && isResetPassword ? (
                            <motion.div
                            ...
                            >
                                ...
                                <h3 className="text-lg font-bold mb-2">Email inviata</h3>
                                <p className="text-sm">Controlla la tua casella di posta per le istruzioni di recupero.</p>
                                <button
                                    onClick={toggleReset}
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4 animate-fade-in">
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                placeholder={isSignUp ? "Crea una password..." : "Inserisci password..."}
                                                className="w-full pl-12 pr-12 py-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-medium"
                                                required
                                                disabled={status === 'loading'}
                                                minLength={6}
                                            />
                                ...
                                <button
                                    type="submit"
                                    disabled={status === 'loading' || !email || (!isResetPassword && !password)}
                                ...
                                >
                                    <span className={status === 'loading' ? 'opacity-0' : 'opacity-100 transition-opacity'}>
                                        {isResetPassword ? 'Invia Email di Recupero' : (isSignUp ? 'Crea Account' : 'Accedi')}
                                    </span>
                                    {!isResetPassword && <ArrowRight size={18} className={`transition-all ${status === 'loading' ? 'opacity-0' : 'group-hover:translate-x-1'}`} />}
                                ...
                                <p className="text-center text-[10px] text-slate-500 mt-4 leading-relaxed px-4">
                                    Procedendo accetti le condizioni di elaborazione dati in cloud per i target giornalieri.
                                </p>
                            </form>
                        )}
                    </motion.div>
