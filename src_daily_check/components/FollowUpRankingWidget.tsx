    ...
    <div className="w-full bg-white/60 dark:bg-slate-900/60 backdrop-blur-3xl rounded-[2.5rem] p-6 border-2 border-black/5 dark:border-white/10 shadow-xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-[10px] font-black uppercase tracking-[0.25em] text-orange-600 dark:text-orange-400">Priorità Follow-up</h4>
        </div>
        <span className="px-2.5 py-1 bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 rounded-lg text-[9px] font-black uppercase tracking-widest">Top AI Picks</span>
      </div>
        ...
        {rankedLeads.map(({ lead, score }, index) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="font-black text-slate-800 dark:text-white truncate">{lead.name}</p>
                {score > 25 && (
              ...
              <div className="flex items-center gap-3 mt-1">
                <span className="text-[9px] font-black uppercase tracking-wider text-slate-400 flex items-center gap-1">
                  <Clock size={10} /> {lead.followUpDate ? format(parseISO(lead.followUpDate), 'd MMM') : 'Nessuna data'}
                </span>
            ...
            <div className="flex flex-col items-end gap-1">
              <p className="text-[10px] font-black text-slate-300 group-hover:text-blue-500 transition-colors uppercase tracking-widest">Dettaglio</p>
      ...
      {rankedLeads.some(r => r.score > 20) && (
        <div className="mt-6 p-4 bg-orange-500/5 dark:bg-orange-500/10 rounded-2xl border border-orange-500/10 flex items-start gap-3">
          ...
          <p className="text-[10px] leading-relaxed font-bold text-orange-700/80 dark:text-orange-400/80 uppercase tracking-tight">
